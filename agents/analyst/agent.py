...
class AnalystAgent(AgentBase):
    ...
    def _explain_recommendation(self, request: BaseModel) -> Explanation:
        ...
        return Explanation(
            summary=(
                "Analyst confidence blends the technical indicators that have"
                f" enough market history for {len(candidate_set.candidates)}"
                " candidates, then gates each result against provider regime"
                " confidence."
            ),
