...
| Law | What the test must prove | Scenario | Test | Status |
| --- | --- | --- | --- | --- |
| ANLZ-OBS-01 | AnalystRun node contains full score breakdown and is reconstructable. | audit | Demoted S169-sweep: `test_analyst_pubsub.py::test_recommendation_result_node_in_graph` asserts only that a `RecommendationResult` node exists carrying a `recommendations` key, on the pub/sub path production does not use. The clause (input candidates, per-ticker scores, regime context, ScoreBreakdown on Recommendation nodes) is unproven. The graph-pull path does persist the whole set as `AnalystRun.recommendation_set` (`poll.py:75`) - a test citing that write would make the row green honestly. | ⬜ |
...
| ANLZ-OBS-04 | Sentiment metrics distinguish scored headline count, word occurrences, and the batch-duplicate weighted article denominator; batch-scoped values say so in the name. | observable | `test_sentiment_weighting.py::test_weighted_article_metric_names_unit_and_batch_scope` | 🟩 |
| ANLZ-OBS-05 | A settled window records the deepest close-to-low fall together with the horizon that produced it. | happy | `test_stop_target_outcome.py::test_settled_window_reports_the_deepest_fall_and_the_horizon_it_covers` | 🟩 |
| ANLZ-OBS-05 | An unsettled window leaves the value absent rather than recording 0.0. | negative | `test_stop_target_outcome.py::test_an_unsettled_window_stays_absent_rather_than_reporting_zero` | 🟩 |
