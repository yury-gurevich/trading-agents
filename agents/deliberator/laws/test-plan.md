...
| DLIB-NEV-08 | Never renders a `PASSED` or `FAILED` verdict for a comparison no agent enforces; every rendered debate-context verdict names its check and enforcing agent, while descriptive evidence has no verdict. | functional | `tests/test_veto_context_verdict_attribution.py::test_no_rendered_verdict_lacks_an_enforcing_check`; `tests/test_veto_context_verdict_attribution.py::test_scaled_mode_renders_no_failed_for_a_preserved_ratio`; `tests/test_veto_context_verdict_attribution.py::test_stop_basis_renders_zero_stop_ratio_as_unavailable`; `tests/test_veto_context_verdict_attribution.py::test_stop_basis_degrades_without_evidence`; `tests/test_veto_context_verdict_attribution.py::test_confidence_floor_names_its_enforcer` | 🟩 |
...
| DLIB-IDM-02 | LLM output bounds are stamped -- role, model, max rounds, **both** prompt halves, response hash and timestamp. | functional | `test_llm_output_bounds.py::test_every_bound_the_clause_names_is_stamped`; `tests/test_llm_ledger_provenance.py::test_the_system_prompt_is_hashed_alongside_the_user_prompt`; `tests/test_llm_ledger_provenance.py::test_two_calls_differing_only_in_system_prompt_are_distinguishable` | 🟩 |
...
| DLIB-OBS-05 | Each `LLMCall` carries the provider's own token counts and declares whether they are vendor-measured or estimated; a truncated or refused completion is still counted. | functional | `tests/test_llm_ledger_tokens.py::test_ledger_records_the_vendors_own_counts`; `tests/test_llm_ledger_tokens.py::test_ledger_stamps_a_word_count_as_estimated`; `tests/test_llm_ledger_tokens.py::test_vendor_and_estimated_rows_are_distinguishable`; `test_ledger_usage_forwarding.py::test_a_truncated_completion_still_records_what_it_burned`; `test_ledger_usage_forwarding.py::test_a_provider_that_reports_no_usage_falls_back_and_says_so`; `test_llm_openai_usage.py::test_cached_tokens_are_subtracted_from_the_input_count`; `test_operator_llm_usage.py::test_operator_calls_carry_the_vendors_own_token_counts`; `tests/test_deliberator_anthropic_cost.py::test_vendor_usage_is_captured_from_the_response` | 🟩 |
| DLIB-OBS-06 | The frozen role system prompt is offered to the provider's prompt cache, and cached input tokens read and written are recorded. | functional | `tests/test_deliberator_anthropic_cost.py::test_system_prompt_is_sent_as_a_cache_marked_block`; `tests/test_deliberator_anthropic_cost.py::test_live_path_uses_the_five_minute_ttl_not_the_one_hour_ttl`; `tests/test_replay_batch_caching.py::test_the_submitted_system_block_carries_a_cache_marker`; `tests/test_replay_batch_caching.py::test_the_batch_path_uses_the_one_hour_ttl_not_the_default`; `test_operator_llm_usage.py::test_the_operator_system_prompt_is_sent_unmarked` | 🟩 |
| DLIB-OBS-07 | Each `LLMCall` records a digest of the code that rendered its prompt; an unrecorded renderer reads as unknown. | functional | `tests/test_prompt_recipe.py::test_changing_a_modules_source_changes_the_digest`; `tests/test_prompt_recipe.py::test_moving_code_between_modules_changes_the_digest`; `tests/test_prompt_recipe.py::test_an_unreadable_module_makes_the_whole_digest_unknown`; `tests/test_prompt_recipe.py::test_the_deliberator_declares_every_module_that_builds_its_prompt`; `tests/test_llm_ledger_provenance.py::test_the_renderer_that_produced_the_prompt_is_recorded`; `tests/test_llm_ledger_provenance.py::test_an_unrecorded_renderer_reads_as_unknown_not_as_this_one`; `test_llm_output_bounds.py::test_the_bound_names_the_renderer_that_produced_it` | 🟩 |
