...
| EXEC-OBS-04 | Deliberation fail-open severity follows recorded posture, so advisory outages are warnings and binding no-verdict buy submissions are errors that acceptance can distinguish. | observable | `test_deliberation_posture.py::test_advisory_posture_submits_unvetoed_buy_and_records_warning`; `::test_binding_posture_blocks_unvetoed_buy_and_records_error`; `::test_fail_open_submission_severity_follows_posture`; `test_trading_deliberation_posture.py::test_advisory_fail_open_fails_without_reason`; `test_trading_acceptance_deliberation.py::test_advisory_veto_that_reviewed_nothing_fails_acceptance`; `test_trading_deliberation_unvetoed.py::test_unvetoed_advisory_buy_breaches`; `::test_a_veto_that_reviewed_nothing_breaches`; `::test_a_partially_degraded_veto_stays_green` | 🟩 |
