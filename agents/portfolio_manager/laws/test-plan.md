...
| PM-NEV-08 | Every usable held issuer contributes by the declared floor-to-ceiling correlation ramp; the cluster including the candidate remains capped by max_correlated_cluster_pct. | boundary | `test_correlation_ramp.py::test_ceiling_correlation_counts_a_held_issuer_in_full`; `test_correlation_ramp.py::test_degenerate_ramp_is_binary_at_the_floor_and_declared`; `test_correlation_concentration.py::test_correlated_cluster_rejects_cross_label_order` | 🟩 |
...
| PM-OBS-03 | An evaluated cluster gate names examined issuers, ramp endpoints, applied positive weights and total weight, plus strongest zero-weight comparisons and minimum overlap. | audit | `test_correlation_ramp.py::test_cluster_ratio_is_recomputable_from_rendered_weights`; `test_correlation_ramp.py::test_staples_near_misses_are_weighted_without_failing_the_cap`; `test_correlation_census.py::test_census_names_the_issuers_it_examined_and_the_ones_it_ruled_out` | 🟩 |
...
| PM-OBS-03 | The census reaches the gate_report detail the deliberator reads, not just the domain object. | audit | `test_correlation_census_gate.py::test_gate_detail_carries_the_census_beside_the_cluster` | 🟩 |
