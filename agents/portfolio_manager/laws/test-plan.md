...
| PM-NEV-04 | min_reward_risk_ratio gate rejects low R/R recommendations, including measured upside below the configured built-ratio floor. | boundary | `test_reward_risk.py::test_rejects_when_reward_risk_below_minimum`; `test_reward_risk_measured_excursion.py::test_default_reward_risk_floor_is_the_corrected_built_ratio_floor`; `test_reward_risk_measured_excursion.py::test_reward_risk_rejects_measured_upside_below_default_floor`; `test_reward_risk_measured_excursion.py::test_reward_risk_between_old_and_new_floor_passes_by_default`; `test_reward_risk_measured_excursion.py::test_reward_risk_floor_is_read_from_the_tunable_value` | 🟩 |
...
| PM-OBS-05 | measured upside below the configured fraction of stop risk is visible and rejectable at the configured floor. | boundary | `test_reward_risk_measured_excursion.py::test_reward_risk_rejects_measured_upside_below_default_floor`; `test_reward_risk_measured_excursion.py::test_reward_risk_between_old_and_new_floor_passes_by_default` | 🟩 |
| PM-OBS-05 | measured reward-risk detail names the target basis and sample inputs and is not labelled structurally fixed. | audit | `test_reward_risk_measured_excursion.py::test_measured_reward_risk_is_not_labelled_structurally_fixed` | 🟩 |
