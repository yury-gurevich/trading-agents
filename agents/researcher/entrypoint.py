"""Researcher agent entrypoint -- PRE_FLIGHT bootstrap and request serving.
...
def build_served_bus(graph: GraphStore) -> MessageBus:
    """Bind researcher RPC capabilities onto a local served bus."""
    bus = InProcessBus()
...
def main() -> None:  # pragma: no cover
    """Activate with master, then serve request-triggered researcher RPCs."""
    import os
    ...
    activate_agent(master_url, "researcher", public_key_pem=pubkey)
    graph = build_graph_from_env()
    bus = build_served_bus(graph)
    serve_loop(consumer_from_env("researcher", graph), bus)
