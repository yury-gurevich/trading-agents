...
Components removed from the repo during the hygiene multipass. Files are **moved** to the
...
| 2026-06-18 | Alembic | `alembic/` (empty `versions/`) | Moved to `trading-agent-del/`. Relational store + Alembic dropped in Sprint 03 (ADR-0001). |
| 2026-06-18 | Local append/WAL store | `data/` | Moved to `trading-agent-del/`. Sprint-02-era persistence, superseded by Neo4j. |

> **Pending decisions (pass 2)** — see `docs/repo-hygiene.md`: the Celery/Redis bus,
> Prometheus/Grafana observability, and the Postgres raw-OHLCV probe are load-bearing (source +
> tests under the 100 % coverage gate) and await an operator call before any retirement.

---

## Shipped (archive: Sprint 36 → P0)
