...
**Last updated:** 2026-09-17 18:43 AEST · **Version:** 0.98.08 · **🟩 S211 REBASED LOCALLY — `reward_risk` now compares measured target upside with stop risk at the corrected 0.80 floor.**
...
🟩 **BUILT and REBASED LOCALLY — [S211](sprints/sprint-211-reward-risk-compares-two-measured-quantities.md) on branch `sprint-211-reward-risk-measured-quantities`, worktree `C:\Users\yury_\Downloads\project\trading-agents-sprint-211-reward-risk-measured-quantities`, base `origin/main` @ `f4cf9285d66844e21a8d6ac35e57aa4232313934`.** Analyst targets now use median trailing favourable-excursion evidence from prior settled windows, `StopTargetEvidence` carries the target estimate/window/sample count, PM `min_reward_risk_ratio` is `0.80` per ADR-0027 Correction / EXP-010, and `reward_risk` no longer reports `STRUCTURALLY_DETERMINED` for measured analyst evidence. Analyst laws moved v1.4 -> v1.5 with new `ANLZ-OBS-06`; PM laws moved v1.6 -> v1.7 with new `PM-OBS-05`; rollups are analyst **26 / 49** and PM **31 / 50**. 🟩 **Local proof so far:** A1/A2 failed red-first before implementation; focused returned-floor guard file **6 passed**; broad changed-area sweep **639 passed**. 🟠 **Not done:** post-rebase redirected `make ci`, push, remote branch CI, `make gate-ran`, merge, full `up` deployment, and live scheduled-run proof that `reward_risk` values vary and no structural label remains.
