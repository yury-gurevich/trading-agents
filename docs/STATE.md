...
**S146 branch update:** 2026-07-28 18:38 AEST · **Version:** 0.80.03 ·
branch `sprint-146-unprotected-position` implements the unprotected-position fix and production
repair. Local production proof found ABT is not skipped by graph state; the remaining blocker is an
Alpaca 403 wash-trade refusal caused by an existing opposite-side order. Four orphan PM-run broker
fills were repaired append-only and are idempotent on a second run. Branch push `dc035ce` had
Security Findings run `30344885685` green and CI run `30344886036` green; no merge to `main`.
