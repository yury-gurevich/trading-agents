...
**Last updated:** 2026-09-16 12:05 AEST · **Version:** 0.98.05 · **🟩 S208 MERGED (`d43280e`), tagged `v0.98.05` and DEPLOYED `s208` — PM risk gates now disclose whether the opposite verdict was reachable, and the correlation gate skips only the unusable pair. 16/16 apps + dispatcher on `:s208`, packs and KEDA unmoved; proven live on `sched-2026-09-15`, and on the first real-debate night without the manufactured stop/regime line the referee **upheld** a name — 3 reviewed, 2 vetoed, WFC through to the broker.**
