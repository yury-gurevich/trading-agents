...
**Last updated:** 2026-09-04 22:35 AEST · **Version:** 0.95.02 · **🚨 The judge was wrong for the first time, and our rendering caused it: a correlation cluster of one is indistinguishable from a check that never ran.**
...
🎯 **MEASURED RESULT — item 47's four objections checked before building, and two are wrong** ([DL-154](design-log.md), 2026-09-04). 🟩 **(a) and (b) REFUTED:** over 120 bars to 09-03, **USB~BAC 0.7639** (correctly clustered), **USB~C 0.5650** and **USB~SCHW 0.3180** (correctly omitted), **AVGO~NVDA 0.4739**, **AVGO~INTC 0.4988** — all at **full 120-bar overlap**, so nothing went unevaluated and `min_correlation_bars` never bound. `C` and `SCHW` *were* measured; they do not correlate with USB at 0.70. **A cluster of one was the correct answer.** 🚨 **The real defect is that we cannot tell that by looking:** `cluster_issuers=AVGO` never reports how many issuers were examined, so *evaluated 27, none above threshold* renders identically to *evaluated nothing* — S183's skipped-vs-passed and [DL-152](design-log.md)'s *producer knew something it never said*, now in a **third** gate, and this time it cost a veto and two nights of not trading. 🟢 **(c) CONFIRMED and understated** — identical `stop_pct=0.0500` for a bank and a semiconductor, `stop_target_mode="flat"`, measured book betas **XOM −0.97 … INTC 3.35**; the fix is **already built and dormant** (S150, [DL-77](design-log.md)). 🟢 **(d) CONFIRMED as description, by design** — the limit is `est_price` ± a flat **50 bps** with no live quote, which is what a limit order is for; the band's size is [DL-76](design-log.md), challenger shipped S149. 🎯 **Item 47 is therefore one small observability fix plus two promotion decisions, not a sprint.** 🪤 **The judge read what we rendered, correctly** — it had been right twice before, and the "verify before building" rule is what stopped two sprints being spent on a working gate.
