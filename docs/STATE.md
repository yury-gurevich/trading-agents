...
**Last updated:** 2026-09-17 21:27 AEST · **Version:** 0.98.08 · **🟩 S211 BUILT LOCALLY — `reward_risk` now compares measured target upside with stop risk and defaults disclosure-only.**
...
🟩 **BUILT LOCALLY — [S211](sprints/sprint-211-reward-risk-compares-two-measured-quantities.md) on branch `sprint-211-reward-risk-measured-quantities`, worktree `C:\Users\yury_\Downloads\project\trading-agents-sprint-211-reward-risk-measured-quantities`, base still `origin/main` @ `f4cf9285d66844e21a8d6ac35e57aa4232313934` after the operator's no-rebase disclosure-only ruling.** Analyst targets now use median trailing favourable-excursion evidence from prior settled windows, `StopTargetEvidence` carries the target estimate/window/sample count, PM `min_reward_risk_ratio` is `0` per ADR-0027 Correction 2 / EXP-011, and `reward_risk` records `comparison=DISCLOSURE_ONLY` by default while explicit positive floors still reject below-floor ratios. Analyst laws moved v1.4 -> v1.5 with new `ANLZ-OBS-06`; PM laws moved v1.6 -> v1.7 with new `PM-OBS-05`; rollups are analyst **26 / 49** and PM **31 / 50**. 🟩 **Local proof so far:** disclosure-return red guard failed first (**2 failed, 11 passed**), focused reward-risk/param suite **18 passed**, broad changed-area sweep **639 passed**, law coverage and touched ruff checks passed. 🟠 **Not done in this file:** final redirected `make ci`, push, remote branch CI, `make gate-ran`, merge, full `up` deployment, and live scheduled-run proof that `reward_risk` values vary and default disclosure reaches the fleet.
