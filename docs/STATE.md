...
`DeployRecord` written after verification), and the **resumed run scored `7/7 stages complete`**
where it had crash-looped at 4/7. Execution wrote `submitted=2 rejected=0 skipped=1`, adopted both
orphaned orders with their real broker ids, and created **no duplicate orders**. **The one success
factor still open** is acceptance itself: `UNPROVEN - completed`, because AMD sell 55 and ABT buy
95 are queued for the 13:30 UTC open. Re-run `accept.py` after the open for the verdict.

**🔴 NEW — DL-73: reconciliation is manufacturing phantom positions.** The same run took the
`Position` count **21 → 23**. `agents/monitor/reconcile.py:108` keys positions
`broker:{ticker}:{qty}:{avg_entry_cents}` and matches on **both** quantity and basis, so any
holding change mints a **new** open `Position` and never closes the old one. SCHW 98 → 196 created
a second open node (**294 graph shares vs 196 held**); ABT 98 → 96 likewise (**194 vs 96**).
Standing: AMD **111 vs 55**, MRVL **two open 44s vs nothing held**. Because ADR-0016 has the
analyst score open held positions, these phantoms get scored, sized, and turned into exit keys —
the exact chain that produced DL-71. This is not a divergence to reconcile once; it is a generator
of divergence, and it makes **DL-71 option B non-optional**.
