...
- **🔴 DEPLOY S151 AND PROVE IT LIVE — the only thing standing between us and a second lost night.**
  S151's success factors are **PROVEN** in the gate: A1 (a `Fill` already carrying
  `broker_status="rejected"` sweeps without raising) and B1 (a raising sweep still writes *and links*
  the snapshot, and the run stops being pending) both pass, and E3 re-proves the nine-stop exemption
  against nine `stop`/`stop_limit` orders plus a droppable non-stop. **None of that is production
  proof.** INTENT: retag the fleet `:s148` → `:s151`, then resume the still-pending
  `run-request:sched-2026-07-30` in a daytime window — **the operator has authorised a daytime
  production run**, and 07-30's close is still the latest close, so the data is current, not stale.
  **Verifiable success factors:** the sweep completes; a `BrokerPositionSnapshot` appears and links;
  `position_sync` marked; the run walks to **8/8** at `ACCEPTANCE PASS`; **all nine stops still
  resting afterwards**. Then a `functionality-checks.md` row. 🕒 **Contingency stands: not deployed
  before 22:30 UTC 2026-07-31 → retag `:s147`** — it predates the sweep entirely and ran 8/8 on
  `sched-2026-07-29`.
