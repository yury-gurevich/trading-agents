...
**Last updated:** 2026-07-22 15:44 AEST · **Version:** 0.71.06 · **🟢 S132 (mutation testing) SHIPPED** (merged `15c23d6`, tag `v0.71.05`; row G Done; +94 mutants killed with cited tests, scoped decision-engine kill-rate 78.47 %→79.87 %, mutmut dev-only + not a CI gate; planning review re-ran the gate 1614 passed / 100 %). **Acting on the S132 findings (operator directive 2026-07-21):** review triage found ~130 survivors in genuinely testable, high-value logic (~50 alpaca money-parsers + ~84 analyst/PM/confidence gate boundaries) — wrongly parked as "rainy day"; the `pragma`-HTTPS transport + string mutants are *justified* exclusions, not "equivalent". Packaged as **S134 (assertion hardening, row K)**, reprioritised to **execute BEFORE S133** (fixes-first; trade-gating code outranks lower-severity bus creds). **S134 round 1 handed back + VERIFIED by planning (2026-07-22):** `make ci` reproduced green (1629 passed / 6 skipped / 100 %), money-parsers 39→7 and PM/gate/reward-risk/sector boundaries killed with real below/at/above assertions citing mutant ids — a genuine partial win, zero production change. **But BOUNCED to round 2 (operator directive):** the ~250 analyst indicator/scoring/pattern/alpha *math* survivors were parked under one blanket "decision-neutral" note (identical on all 274 analyst rows) when a sample is deterministic pure math (ATR/stochastic/choppiness/pattern/alpha/scoring) — assertion-weak, **not** equivalent (the exact hand-wave row K exists to stop). Round-2 spec **R1–R6 committed (`85c410e`)**: kill the killable with known-input→known-output assertions + **per-mutant-justify** any residual (no blanket notes), re-run the S132 scope with a per-module before/after table, fix the report's phantom `test_alpaca_helpers.py`, **revert row K to partial** until proven. **R2 done + VERIFIED (`make ci` 1650 passed / 6 skipped / 100 %, `bb8976e`):** killed 122 of the analyst residual (**249→127**) with a real per-module before/after table (`scoring` 24→0, `technical_rules` 17→3, `alpha_features` 32→6, `indicators_pattern` 62→35), kept row K honestly **Partial**, removed the phantom `test_alpaca_helpers.py`, **no blanket labels** — the honest partial the bounce was meant to produce; overall kill-rate 79.87 %→80.82 %→**82.71 %**. **R3 packaged (`7339a0a`, operator directive "round 3 then merge Done"):** close the **127 residual** so every survivor is **killed or individually dispositioned — zero un-triaged** (a per-mutant `killed | equivalent(reason) | wording-exclusion(reason)` list is the anti-hand-wave artifact); then row K → Done and **planning merges 0.71.06; S133 → 0.71.07 follows.** **🟢 S134 SHIPPED (merged `d831260`, tag `v0.71.06`; GitHub CI + CodeQL + image build + dependency graph all green) — row K CLOSED honestly.** R3 forced **all 127** residual analyst survivors into an auditable per-mutant disposition: **107 killed, 12 individually justified equivalents, 8 named wording exclusions, 0 un-triaged** (`round-3-dispositions.csv`; planning checked the anti-template gate — 20 non-killed rows, 20 *distinct* reasons). Scoped decision-engine kill-rate **79.87 %→84.36 %** (5,678/6,731). Test + docs only — no production source changed, no `pragma` removed (81/81 vs main), mutmut stays manual and out of `make ci`. Planning reproduced `make ci` on the branch (**1692 passed / 6 skipped / 100 %**) and on the merge result. Row K moved to the backlog *Done* list; Open now holds only row I. **The three-round verify-bounce-verify arc is the point: a report acted on, not filed. Next: S133 (Service Bus SAS) → 0.71.07.** `mutmut` is WSL-only (refuses on native Windows), so the round-1 delta is the coder's Linux measurement; planning reproduced CI + read the survivor pool from the S132 CSV. Prior milestone still stands: **THE STACK IS VALIDATED IN PRODUCTION.** `sched-2026-07-20` (dispatcher `dispatcher-cron-29743110` Succeeded on `:s130`) ran **7/7 → ACCEPTANCE PASS** with **ZERO `*_degraded` notes** — the first fully-fed scheduled run since 07-07. S128 proven in-fleet: all four enrichment feeds populated (1867 headlines; earnings-window filter actually fired), sentiment restored, and the analyst scored on **full signal** — flipping the chronic all-reject no-trade signature into **5 buys** (USB/BAC/PYPL/WFC/ABT, conf 0.61–0.68 lifted over the 0.600 floor by sentiment). S130 hardened DHI runtimes booted and ran the whole chain; 0 Escalations. Fleet on `:s130` (built `d0b0d3a`). **P12 clean-news runway is now live** (first accumulating night). Pending: the standing broker-divergence Flags (07-09/07-14/07-15) still await operator ack; S131 per-role DSN flip not yet applied (this run used the shared DSN).
