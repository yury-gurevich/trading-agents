...
**Two more packaged the same day, both buildable without an LLM.** **[S186](sprints/sprint-186-a-headline-about-twenty-companies-is-not-news-about-one.md)** — a headline the vendor filed under twenty tickers counts once, in full, for each. ✅ **The open shape question is closed by measurement, not taste** ([DL-127](design-log.md)): `1/n` down-weighting silences **0** tickers where dropping silences **4**, discards **0 %** of slots where dropping discards **23.4 %**, and its worst downstream confidence shift is **0.034 vs 0.065**. 🚨 Two carried queue numbers were wrong and are corrected (19 % → **23.4 %**; "1 ticker" → **4**). **[S187](sprints/sprint-187-a-parameter-is-declared-once.md)** — PARAM tables and code drift **in both directions** across three agents: the scanner ignores a law row the *analyst* honours, two provider settings are in no law at all, `deliberation_grace_seconds` has no row, and `execution.stage` is **retracted** (already declared). 🎯 Its durable half is a `make ci` check — the second audit to find this class. 🟢 **Both are handover-ready as of 2026-08-24, and preparing them found two defects in the specs themselves.** (1) S186's A6 cited a `sched-2026-08-21` fixture **that did not exist** — a worktree has no `.env`, so its one tripwire was unrunnable; the run's whole news batch + analyst baseline are now committed as `agents/analyst/tests/data/news_sched_2026_08_21.json`, **proven** sufficient: 28/28 stored confidences recompute to 3 dp and `KO` lands on 0.599 without graph access. (2) 🚨 **The `1/n` denominator's scope was never stated and it flips the result** — *[measured 2026-08-24]* counted over the 98 news tickers `KO` is **0.599**, counted over the 28 scored candidates it is **0.600** and the sprint's only floor crossing vanishes. Now specced, and [DL-127](design-log.md) amended. S187 carried two drifted citations (`settings.py:122`→123, `laws.md:310`→**326**, where 310 is a bare `}` above the `execution.stage` row the spec says **not** to touch). `make ci` exit 0 on the fixture commit — 2378 passed / 100.00 %. **No version bump: no package behaviour changed.**
