...
**Last updated:** 2026-09-05 11:18 AEST · **Version:** 0.95.03 · **🚨 The judge was wrong for the first time and our rendering caused it — fixed: an evaluated gate now says how much it examined.**
...
🟩 **BUILT — [S197](sprints/sprint-197-a-gate-says-what-it-examined.md) `0.95.03`, the rendering defect [DL-154](design-log.md) found is closed** ([DL-155](design-log.md), 2026-09-05). 🟩 **The measurement first:** over 120 bars to 09-03, **USB~BAC 0.7639** (correctly clustered), **USB~C 0.5650** and **USB~SCHW 0.3180** (correctly omitted), **AVGO~NVDA 0.4739**, **AVGO~INTC 0.4988** — all at **full 120-bar overlap**, so `min_correlation_bars` never bound and **a cluster of one was the correct answer**. 🚨 **The defect was that nothing could tell you that:** `cluster_issuers=AVGO` never reported how many issuers were examined, so *evaluated 8, none above threshold* and *evaluated nothing* were the same string — S183's skipped-vs-passed and [DL-152](design-log.md)'s *producer knew something it never said*, in a **third** gate, this time costing a veto. 🟩 **Now fixed:** the outcome renders `examined_issuers`, `correlated_issuers` **with measured values**, the three strongest `below_threshold_top` near misses, `correlation_threshold` and `min_pair_overlap_bars` into the same `detail` the deliberator reads (`context_pm.py:95`, no contract or vocabulary change — **image-only retag**). New clause **`PM-OBS-03`**, PM laws **v1.4**, rollup **29 / 48** derived by the gate, not by arithmetic. **Two guards planted, watched red, restored.** `make ci` exit 0, **2508 passed**, 100.00 %. 🟠 **What remains of item 47 is two promotion decisions, not code:** **(c)** S150's volatility-scaled stops, built and dormant — the only queue item with a measured case behind it, and **(d)** the 50 bps limit band, confirmed **by design**. 🪴 **Fixing the rendering does not predict a trade** — objections (c) and (d) were CONFIRMED and are untouched; the next no-trade night will now name which one did it.
...
🎯 **Next — the queue has a date now: the operator wants the debt gone by the **end of next week**, i.e. **Friday 2026-09-18**, not 2026-09-11 (clarified 2026-09-05).** **15 open, and the top of the list is a decision rather than a build:** promote **S150's volatility-scaled stops** — the only queue item with a measured case (`stop_pct=0.0500` is 2.4 ATRs for BAC and 0.6 for MRVL; book betas **XOM −0.97 … INTC 3.35**), a **built, dormant** implementation ([DL-77](design-log.md)) and a named trap; it is a `stop_target_mode` flip in `trading_tunables.json` plus a verification run, not a sprint. Then **item 9 / S173**, the verdict-quality gate: the instrument for the `effort`/`max_rounds` cost question and the only way to read the **56 %** self-agreement. 🪴 **Two design-log statuses were found stale on 2026-09-04, both claiming OPEN for work already shipped** — [DL-95](design-log.md) (fixed by S163 the day it was filed; 28 days stale) and [DL-63](design-log.md) (option C shipped as `trading_tunables.json` in S169). **Treat every remaining OPEN status as suspect until measured**; DL-08a (Neo4j, deleted in S118) and DL-80 are the next two expected dead.
