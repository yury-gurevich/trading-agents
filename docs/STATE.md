...
- **🔴 S151 is with codex — an outage fix, and it is time-boxed.** INTENT: append-safe drop
  evidence (`drop_reason` + the `BrokerOrderStatus` node the sweep *already* writes correctly on the
  next line), per-order containment inside the sweep, and a sweep that can never cost the snapshot.
  **Verifiable success factors:** `make ci` green at 100.00 %; four remote gates green on the branch
  tip; the 🎯 A1 regression (a `Fill` already carrying `broker_status="rejected"` sweeps without
  raising) and the 🎯 B1 containment test (a raising sweep still writes and links the snapshot) both
  passing; nine-stop exemption re-proven (E3). Then `v0.84.01`, retag `:s151`, and — **the operator
  has authorised a daytime production run** — resume the still-pending `run-request:sched-2026-07-30`
  rather than waiting for the cron: 07-30's close is still the latest close, so the data is current.
  🕒 **Contingency decided in advance: if S151 is not deployed before 22:30 UTC 2026-07-31, retag to
  `:s147` for the night** — it predates the sweep entirely and ran 8/8 on `sched-2026-07-29`.
- **⚠️ The first corrected sweep will report ~10 drops, not ~3 — that is not the drop rate.** Ten
  `Fill` nodes from the 07-22/07-23 cancelled runs have been sitting in the colliding state
  (`broker_status=rejected`, `drop_reason=None`) and all of them clear on the first pass that
  completes. **ADR-0018's real per-session drop rate is only measurable from the second clean run
  onward** — and that baseline is still owed. **Both challengers (S149 tolerance, S150 stops) stay
  off until it exists**; they are measured *against* the flat champion's live behaviour, so
  promoting before it exists would compare against nothing.
