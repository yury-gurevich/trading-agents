...
**🟠 Retag the fleet — the fix is merged, production is not running it.** S145 is **SHIPPED**
...
Security Findings green on the merge tip, `Build and push agent images` green on `main`, images in
GHCR, `v0.80.02` tagged. **The running fleet is still `:s143`, which predates the fix**, so the
22:30 UTC dispatcher still fires into a bricked execution agent — the poisoned `pm-run-df925eea…`
is returned first by `find_pending` on every poll. **Nothing in production changes until the
retag.** It outranks everything queued below.
...
scale-to-zero, idle ≈ $0. **S145 has shipped and its images are built, but the fleet has not been
retagged — so it still fires into a bricked execution agent.** **Both scoped-credential
...
- **DL-71 option B — reconcile the book before the analyst decides** (the priority successor). The
