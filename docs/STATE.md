...
**🔴 S145 is packaged and is the only thing that matters** —
[sprint-145-exit-replay-append-safe](sprints/sprint-145-exit-replay-append-safe.md) (0.80.02,
[DL-71](design-log.md) option A). **Execution is bricked in production** and will fail every run
until this merges and the fleet is retagged: the poisoned `pm-run-df925eea…` is returned first by
`find_pending` on every poll. This is a fix and it outranks everything queued below.

**INTENT — success factors (LAW-02).** One attempt = one immutable `Fill` node (broker
`client_order_id` unchanged — the 0.74.01 oversell guard stands); a `filled` exit is never
re-issued, and the skip is a visible `Fault`; a per-intent failure degrades to a per-intent fault
so one ticker cannot cost three stages (DRIFT-014 / S128 restated for order submission); the two
orphaned live orders end up with Fill nodes matching **real** broker state, adopted not fabricated.
**Proven when:** `make ci` 9/9 at 100 %, all four remote gates green *before* a local merge, every
new check observed **failing** on a planted violation (DL-70), and a resumed run scoring **7/7**
with MRVL out of the book and SCHW carrying its stop. Sequenced *before* S144's dated vocabulary
...
**Fleet:** standing on `:s143` (verified 2026-07-28 — all 13 apps + dispatcher on the same tag),
self-driving in paper mode — the calendar-gated 22:30 UTC `dispatcher-cron` fires nightly, KEDA
scale-to-zero, idle ≈ $0. **It fires into a bricked execution agent until S145 ships and the fleet
is retagged.** **Both scoped-credential
...
- **DL-71 option B — reconcile the book before the analyst decides** (the successor to S145). The
  ...
  to make on top of a live outage. **Deferred, not rejected.**
