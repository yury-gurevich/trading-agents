...
**Last updated:** 2026-09-05 12:40 AEST · **Version:** 0.96.00 · **🚨 A promotion that had been “pending evidence” since S150 could never have moved: the evidence had no producer. Both halves now ship.**
...
🟩 **BUILT — [S198](sprints/sprint-198-a-stop-we-never-measured.md) `0.96.00`: the stop mode flips, and the book starts recording whether it was right** ([DL-156](design-log.md), operator decision 2026-09-05). 🚨 **The finding that changed the decision:** ADR-0013 makes the S150 promotion an operator flip *on evidence*, and measured over **269 recorded recommendations** the evidence column was structurally empty — `known_outcomes` **0**, because `stop_target_observed_drawdown_pct` is declared in the vocabulary and read by `compare_stop_targets.py` and **no agent writes it** (item 49; [DL-152](design-log.md)'s pattern in a fourth shape — a declared property with *no producer at all*, which reads exactly like “no outcomes yet”). 🟩 **The same measurement refuted [DL-77](design-log.md)'s named trap:** RR pass rate **100 % in both modes** (flat **5.00 / 5.00 / 5.00**, scaled **3.00 / 5.45 / 8.00**), so widening stops does **not** silently stop AMD/MRVL/HPE being traded. 🟩 **Shipped together:** `ANALYST_STOP_TARGET_MODE=scaled` in the pack **and** `ANLZ-OBS-05` — the deepest close-to-low fall over a **declared horizon**, append-only, **absent** when unsettled and a **real zero** when the name did not fall. Analyst laws **v1.3**, rollup **25 / 48**; two guards planted, watched red, restored; `make ci` exit 0 (2524 passed, 100.00 %). 🪴 **The flip is live, not vindicated** — the first settled windows will be short and few; state the denominator before acting on them. 🚨 **Deploy is a full `up`, not a retag** (the vocabulary moved), and it carries S197 too. 🎯 **Queue: empty by Friday 2026-09-18** (operator, 2026-09-05 — not 09-11). Next: **item 9 / S173**, the verdict-quality gate — the instrument for the `effort`/`max_rounds` cost question and the only way to read the **56 %** self-agreement.
