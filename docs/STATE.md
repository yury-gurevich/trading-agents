...
**Last updated:** 2026-09-05 11:55 AEST · **Version:** 0.95.03 · **🚨 The stop promotion cannot be decided on our own book: the property that would settle it is declared, read, and written by nobody.**
...
🎯 **Next — the queue has a date: the operator wants it empty by the **end of next week**, **Friday 2026-09-18**, not 2026-09-11 (clarified 2026-09-05). 15 open.** 🚨 **The top item is a decision, and measuring it changed what the decision is.** Promoting S150's volatility-scaled stops was ranked first as "a config flip and a verification run". Measured 2026-09-05 against the live graph over **269 recorded recommendations** (`scripts/compare_stop_targets.py`): flat **5.00 / 5.00 / 5.00**, scaled **3.00 / 5.45 / 8.00**, and **RR pass rate 100 % in both modes** — so [DL-77](design-log.md)'s named trap (widen the stop, silently stop trading AMD/MRVL/HPE) **does not bite**. 🚨 **But `known_outcomes` is 0 and always will be:** `stop_target_observed_drawdown_pct` is declared in the vocabulary and read by the comparison script, and **nothing in the fleet writes it** — only two tests do (**item 49**, filed 2026-09-05). ADR-0013 makes promotion an operator flip *on evidence*, and the only touch-rate evidence is DL-77's external 65-session backtest from 2026-04, **not our trades**. 🎯 **So the operator's choice is: flip on the backtest, or record outcomes first.** Then **item 9 / S173**, the verdict-quality gate — the instrument for the `effort`/`max_rounds` cost question and the only way to read the **56 %** self-agreement. 🪴 **Treat every remaining design-log OPEN status as suspect until measured** — two were found stale on 2026-09-04 ([DL-95](design-log.md), 28 days; [DL-63](design-log.md), shipped in S169); DL-08a and DL-80 are the next two expected dead.
