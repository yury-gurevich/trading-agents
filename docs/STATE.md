...
**Last updated:** 2026-07-30 13:47 AEST · **Version:** 0.83.00 (S149 branch; fleet still `:s147`) · **🟢 S147 IS PROVEN IN PRODUCTION — and it fixed the thing it predicted it would.** `sched-2026-07-29` ran on `:s147` and scored **`ACCEPTANCE PASS`**; the head sync executed for real (`position-sync:sched-2026-07-29`, `status=fresh`, 22:30:43). **ABT received its protective stop.** The named success factor was falsifiable and it held: the book is now reconciled *before* execution reaches stage 5, so `place_broker_stops` saw ABT 191/191 instead of the stale 96/191 that had been silently skipping it. **All nine held positions now carry a stop at the correct quantity, and there are zero open non-stop orders** (ABT 191, BAC 503, CSCO 177, HPE 229, MDT 233, PYPL 175, SCHW 196, USB 478, WFC 348). MDT cleared by a different route than predicted — its blocking buy filled at the open (118 → 233), removing the wash-trade refusal — and that fill is ADR-0018's cost paid one more time, 115 shares bought at a price nobody evaluated. · **🟢 S148 MERGED (`314fb8b`, 0.82.00, `v0.82.00`) — NOT DEPLOYED; the fleet still runs `:s147`.** ADR-0018 implemented: bounded same-session limit orders (`order_price_tolerance_bps=50`, `day` TIF, paper/live parity), a head-of-run stale-order drop sweep that runs *before* the snapshot so stop placement sees an unblocked broker, stop exemption by **both** broker metadata and graph facts, visible dropped-decision evidence, reporter drop metrics, and A5 unprotected+dropped audit detection. Gates green on the tip `73f0132` (CI `30445292405`, Security Findings `30445292170`); `make ci` re-verified independently on a clean worktree at 1929 passed / 100.00%. **DRIFT-026 opened — the fourth consecutive sprint to find execution's LOCKED constitution behind its code** (024 broker stops, 025 `BrokerPositionSnapshot`, 026 the tolerance tunable and drop semantics). That is now a pattern, not a backlog row: it wants one law-amendment cycle covering all of them. · **🟢 S149 IMPLEMENTED LOCALLY — the flat tolerance is not a neutral parameter** ([sprint-149](sprints/sprint-149-volatility-scaled-tolerance.md), feat → 0.83.00, branch `sprint-149-volatility-scaled-tolerance`, `origin/main=e8bcca1`, local `make ci` exit 0 at 1950 passed / 100.00%; remote CI `30512414390` and Security Findings `30512414394` green on implementation tip `0db7790`). Measured against 60 sessions of real overnight gaps with *directional* refusal: 50 bps refuses **25 % of SCHW buys but 52 % of AMD and 48 % of MRVL** — one median gap for SCHW, one *fifth* of one for MRVL, so a single number encodes two different policies depending on which ticker it lands on. Ships a volatility-scaled challenger off `atr_pct` (already in `quant_metrics`, so the analyst is unchanged), **off by default**, with the counterfactual recorded so promotion is an operator config flip decided on evidence (ADR-0013). [DL-76](design-log.md) also records the larger finding left unfixed: **`suggested_stop_pct` is flat across the whole book too** (`regime.base_stop_loss_pct` for every buy), which is the same defect one layer over and governs risk rather than execution price. · **🟢 THE EXIT-REPLAY OUTAGE IS FULLY CLOSED.** `sched-2026-07-27` — the run that crash-looped for two hours — now scores **`ACCEPTANCE PASS`**, which was S145's last outstanding success factor. `confirm-s146-20260728` is **PASS**; last night's `sched-2026-07-28` ran **7/7** on `:s146`. **S146 shipped, deployed, proven** (0.80.03, merged `7b06662`, `v0.80.03`, fleet retagged `:s145` → `:s146`, 14/14 verified on tag, `DeployRecord …:s146:7b06662` written after verification). **S145's completed-exit skip fired in production for real** — `CompletedExitReplaySkipped … AMD position_ref=22d71d0d3acc0586` — the exact defect that bricked the fleet, provably contained on a live scheduled run. **ABT's missing stop was never a code bug:** Alpaca refuses it as a wash trade (`code 40310000`, `"opposite side market/stop order exists"`) against `fd1f1c2c` — the **ABT buy orphaned by the 07-27 crash**. S145's orphan and S146's unprotected position were the same event two hops apart. What S146 fixed is the **silence**: the refusal is now retried every run and re-surfaced as an `UnprotectedPosition` Fault, where before it was recorded once and forgotten. · **🔴 ADR-0018 DECIDED (2026-07-29) — the largest measured cost in the system.** Every order is a market order submitted after the close and filled at the next open, at a price nobody evaluated: **≈ −$2,850 across two exits** (MRVL −$1,330.12; AMD −$3,515.60, of which **≈ −$1,515 is pure overnight gap** — decided at `est $494.90`, filled at `$467.35`). [ADR-0018](decisions/0018-decision-validity-same-session-or-dropped.md) closes it: **a decision is valid for one session — fill it or drop it.** Bounded price tolerance, cancel-unfilled at session end, dropped decisions recorded as a visible `Fault`, **resting broker stops exempt** (they are risk instruments, not decisions). S148 is the active build. · **✅ RESOLVED 2026-07-30 — the two unprotected positions now carry stops.** This entry previously read *"TWO POSITIONS STILL CARRY NO STOP — ABT 191 sh and MDT 118 sh"* and recorded that my 07-28 prediction had **not** been met. Kept as a correction rather than deleted, because the diagnosis it forced is what fixed it: the position book was never at fault (9/9 matching the broker); the cause was the **same stale-book ordering defect S147 fixes, one stage further down** — `place_broker_stops` runs at stage 5 off a book the monitor only heals at stage 6, so it saw ABT 96 against a broker holding 191 and the quantity guard skipped it. The `:s147` head sync closed that gap and ABT's stop went on at the first run. MDT cleared separately when its blocking buy filled. **Verified live 2026-07-30: nine held, nine stops, zero unprotected.** · **⬜ DRIFT-024 open / ⬜ DRIFT-026 open** — execution's LOCKED constitution still needs a later law amendment for broker-stop state/fallback parameter and for ADR-0018's bounded-order tolerance/drop semantics. · **📁 Detail for the S128–S146 arc — the outage narrative, S144's scheduled outage caught before it fired, Opus 5 at max effort, the 07-24 loop closing, the hardening run — moved to [state-archive/STATE-06.md](state-archive/STATE-06.md)** on 2026-07-29, when this line had reached 17,904 characters.
