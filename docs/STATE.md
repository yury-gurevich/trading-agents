...
**Last updated:** 2026-09-05 17:40 AEST · **Version:** 0.97.00 · **🎯 The veto's self-agreement is a measured number for the first time — and its interval says the 56 % that drove three decisions could never distinguish chance from acceptable.**
...
🟩 **PROVEN RESULT — [S173](sprints/sprint-173-a-verdict-must-be-reproducible.md) Part A MERGED (`0.97.00`), 2026-09-05 — "does the veto agree with itself" is now a number a script prints, with its denominator, its exclusions and a confidence interval** ([DL-158](design-log.md), [DL-159](design-log.md)). 🎯 **Validated against ground truth, which is the only reason to believe it:** the harness re-derives [DL-104](design-log.md)'s hand-computed self-agreement **exactly** — `matched=9; compared=16; rate=56.25%`, through the same code path every other agreement number goes through. 🚨 **And the interval it adds changes what that number means: `ci95=[33.18%, 76.90%]` contains both 50 % and 75 %**, so the 56 % cited in three decisions (DL-105's sweep, the 2026-08-13 timeout incident, DL-140's rejected routes) can distinguish neither "chance" nor "acceptable". 🚨 **Two things measured before any code was written, both of which changed the design:** a debate **cannot** be one batch of five requests — `render_debate_prompt` interpolates the transcript, so turn N+1's prompt contains turn N's answer, and the harness runs **five dependent rounds**, each one batch across every debate; and the spec's `custom_id` is illegal on the wire, so the readable label is truncated and re-uniqued by a digest of the whole label. 🚨 **The exclusion predicate is not a rounding correction: 83 of 329 approved orders on the spine are fail-opens — 25.2 %**, recorded as `verdict: "uphold"` and indistinguishable from a real ruling; every one of the other 246 has a real verdict, so the corpus splits exactly two ways. The recorded distribution is **61 % `revise` over 246**, not DL-104's 78 % over 58. 🪤 **The two graph stores return different Python types for one property** — in-memory freezes props (`dict` → `mappingproxy`, `list` → `tuple`), Postgres returns plain JSON, so an `isinstance(x, dict)` guard passes in production and fails in tests. Caught by a red test. 🟩 **DL-70 sweep: 10 mutations planted, and the first pass found two guards that did not guard** — `wilson_interval`'s clamps returned an in-range interval from a corrupted denominator (a clamp cannot correct a real Wilson bound, only hide a broken one; removed, and the interval is now pinned by value), and `no_counterpart` could be zeroed because its only covering test used a case where zero is correct. Re-run: **10 of 10 caught**. 🟠 **No clause moved, and that is the honest outcome:** `DLIB-IDM-02` was the candidate and measuring it produced **[DRIFT-056](laws/drift-register.md)** instead — `prompt_hash` covers the *user* string only, so two calls with different system prompts are indistinguishable in the ledger. 🟩 **Proven:** `make ci` exit 0 (2592 passed, 6 skipped, 100.00 %). **No deploy — read-only offline tooling, decision 1 taken as NOT PERSISTED**, so no law cycle, no vocabulary move, no fleet reach. 🚨 **Part B is unrun and it is a spending decision, not a build:** control arm only ≈ **$30**, three arms × 3 ≈ **$90**, three arms × 5 ≈ **$151**. 🪤 **The control arm is not the cheap cut** — dropping arms B and C still leaves a usable result; dropping arm A leaves three numbers with nothing to compare them to.
...
🎯 **Re-ranked 2026-09-05, after S173 Part A merged.** **15 items open** against the operator's **Friday
2026-09-18** empty-by date, and **item 9 is no longer a build** — the instrument exists and is validated, so what
is left of it is a spending decision the operator owns: **$30 / $90 / $151** for one / three / three-with-five-repeats
arms ([DL-158](design-log.md)). Behind it: item 47's remaining objection, item 48's silent pack-key shadowing.
