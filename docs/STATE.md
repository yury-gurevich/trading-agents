...
**Last updated:** 2026-09-18 09:52 AEST · **Version:** 0.98.08 · **🚨 Four zero-fill sessions: the referee's three grounds are now all measured true-but-inert (EXP-008/011/012), and the open question is what its middle verdict should bind — [DL-173](design-log.md).**
...
🚨 **DECISION OWED — [DL-173](design-log.md): the referee's middle verdict has no middle action.** Four consecutive scheduled sessions closed with **zero fills** (09-14, 09-16 ×2, 09-17) while every stage ran green and acceptance passed; on `sched-2026-09-17` the PM approved 3 and the deliberator vetoed all 3. 🎯 **Three independent experiments now say its grounds are correct and its fixes do not pay:** `reward_risk` cannot fail *and the ratio does not predict returns* ([EXP-011](research/experiments/EXP-011-regime-markov-and-barrier-calibration.md)); the correlation gate is a 0.70 cliff *and no cutoff from 0.70 to 0.50 changes a single trade* ([EXP-008](research/experiments/EXP-008-correlation-cutoff-replay.md)); sizing caps dollars not risk *and fixing it costs $11,619 over ten years* ([EXP-012](research/experiments/EXP-012-volatility-sizing-ten-year-replay.md), 100 % of bootstrap resamples negative). 🪤 **The mechanism is in the code, not the model:** `revise` (164 all-time) and `overturn` (22) are collapsed to one action at `agents/deliberator/review_batch.py:87`, so a critique of *gate design* is spent rejecting a *specific trade* — the only channel the referee has. A tempting alternative explanation was **measured and rejected**: only **1** parse-failure `revise` exists across all 63 runs, so the vetoes are real judgements, not an outage. **OPEN — authority question, operator's call (charter OUT-2); no implementation proposed or authorised.**
