...
**Last updated:** 2026-09-16 18:46 AEST · **Version:** 0.98.06 · **🟩 S209 MERGED and DEPLOYED as `s209` — the stale-order sweep now compares stop identity, not liveness views; `EXEC-OBS-05` is v1.6 and `DRIFT-064` records the stale clause. Local/remote gates, image build, merge, retag, and `DeployRecord` are proven; the future fired-stop live proof is still owed.**
