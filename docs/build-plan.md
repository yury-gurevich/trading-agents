...
> (This split exists because the two docs drifted once — see CLAUDE.md "Single live tracker".)
...
| P12 Sentiment (champion–challenger) | **SHIPPED — closed 2026-06-27 (DL-32).** Champion (LM lexicon) is **binding** (`sentiment_weight=0.20`, used in `analyst/domain/scoring.py`); provider + FinBERT challengers run **advisory**; news feed live; forecaster **activated** as an advisory shadow stage (DL-30). All code is in the live decision path. The **one** open piece — run the 3-scorer scorecard on forward returns → promote a challenger via the P10 gate (`forecaster.sentiment_scorecard` harness, callable but never triggered) — is **DEFERRED, not abandoned**: it needs a live **news-accrual runway** that is not being accrued under the etalon-first pivot. Nothing removed. ADR-0002. |
| P13 Cross-asset & macro signal graph | **DEFERRED — not started (zero code; DL-33).** Sector contagion + signed tariff/sanction event propagation over Neo4j. Gated on three things, none active: P12's deferred news runway, **premium relationship data** (supplier/exposure edges), and the etalon-first pivot. The plan's highest-ambition phase — kept **intact as a future direction**, but in no current queue. ADR-0002. |
