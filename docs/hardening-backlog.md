...
| R | **The 100 % coverage floor is partly satisfied by packages being absent, not by tests.** Five lazy-SDK guards raise `ConfigurationError` on `ModuleNotFoundError` (`kernel/llm_openai.py`, `kernel/llm_anthropic.py` — **both moved from `agents/` by [S222](sprints/sprint-222-one-llm-adapter-lives-in-the-kernel.md) 2026-09-21, and the operator's second copy is now deleted rather than relocated**; `agents/forecaster/finbert.py`, `lightgbm_model.py`). Only two are now exercised by a test. The other three are **covered because CI does not install those optional extras**, so the `except` line executes for real. | Coverage that comes from the environment rather than a test disappears the moment the environment changes: installing an extra locally drops coverage below the floor and reds `make ci` on an unrelated branch — measured 2026-08-14, `llm_openai.py` at **93.94 %** on a machine carrying `openai`. It also means the guards themselves have never been proven; the one that *had* a test passed for four months on a monkeypatch of `builtins.__import__` that `importlib.import_module` does not consult. | Trigger: the next time an optional extra is added to the dev environment or CI, or any sprint touching `agents/forecaster/` (which owns two of the three). Fix shape is known and cheap — `chore-a-guard-must-be-exercised` did it for the deliberator's two adapters with a shared `_hide_sdk` helper. |
| T | **A hand-rolled probe reads "didn't look" as "nothing there", and the repo usually already has the target.** Measured three times in one session, 2026-09-13: (1) `gh run list --commit <sha>` returned nothing for a commit whose gate was green — `make gate-ran` answered it immediately, and CLAUDE.md already warns *do not hand-roll the query; run the target*; (2) a stage watcher matched nodes by run-id substring, which ticked `MonitorRun` off the wrong node **and could never match the right one** (real keys are `monitor-run-<uuid>` and contain no run id) — `orchestration.batch_chain.walk_chain` walks the provenance edges and exposes the canonical 8 stages; (3) a `DeliberationRun` audit read `props['llm_call_keys']`, which is not a property at all but a parameter creating `PRODUCED_BY` **edges** — filing a false defect (work-queue item 51, retracted the same day) that repeated [DL-73](design-log.md) exactly, against the explicit warning never to audit the graph on raw props. | **All three failed as a false negative that looks like patience.** A probe that finds nothing is indistinguishable from a probe pointed at the wrong thing, so silence reads as *still running* or *nothing wrong* — the most expensive shape, and the same DL-57 *didn't look vs looked and found nothing* pattern hardening row O records at document level. Instance (3) cost a filed defect and a retraction; (1) cost a 20-minute wait; (2) would have certified a run complete that had not completed. | **Now, as a standing rule rather than a task:** before writing any probe, grep for an existing target — a `make` target, a `scripts/` entry point, or an `orchestration/` walk — and prefer it even when it looks heavier. When a probe must be hand-rolled, it has to distinguish *searched and found nothing* from *did not search*: emit on every terminal state, assert the shape it queries actually exists, and never let an empty result path exit 0 silently. |
