...
| **K** | Assertion-strength gap in trade-gating + money-parsing code (surfaced by S132 mutation) | **Done in S134.** The S132 rerun baseline moved from 5,376/6,731 killed (79.87%) to 5,440/6,731 killed (80.82%), with survivors down from 1,178 to 1,114 and no timeout/suspicious/skipped/incompetent mutants. The money-parser pure-helper bucket dropped from 39 actionable survivors to 7 equivalent `_fill_from_order` default-normalization residuals; the non-equivalent analyst/PM gate-boundary mutants selected from the S132 work-list were killed with explicit boundary assertions. | Permanent justified non-targets: `# pragma: no cover` Alpaca HTTPS transport remains covered by live/paper broker proof rather than HTTP-mock assertion tests; string/render/audit/context survivors remain excluded to avoid over-specification. Evidence: [S134 assertion hardening report](reports/sprint-134-assertion-hardening/README.md). |
