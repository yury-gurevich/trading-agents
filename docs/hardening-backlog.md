...
Opened 2026-07-23 — **gates or agent authority**, found while shipping ADR-0016. **L closed 2026-07-27** (see Done); M and N remain. **O and P added 2026-08-01**, **Q added 2026-08-02** — both found while reconciling the law book in S152, both pre-existing and deliberately outside that sprint's scope. **R and R² added 2026-08-02** by `chore-exec-fail-03-coverage` — O, R and R² are one defect at three depths: clauses with no row (O), roll-ups that disagree with the rows (R), and rows whose cited proof does not exist (R²). Close them in one sweep.
...
| R | **`ledger.md` and `laws/INDEX.md` disagree with the test-plans they summarise, for 8 of 14 agents.** Counted 2026-08-02 (🟩 rows in each `agents/*/laws/test-plan.md` vs the two roll-up tables), *after* this chore took execution to 31/31/31. `portfolio_manager` **33 / 24 / 24**, `monitor` **24 / 19 / 19**, `curator` **24 / 22 / 20**, `supervisor` **22 / 21 / 21**, `reporter` **21 / 19 / 19**, `forecaster` **18 / 16 / 15**, `operator` **15 / 14 / 14**, `researcher` **19 / 19 / 18** (INDEX only). Agreeing: provider, scanner, analyst, deliberator, execution, master. | `ledger.md` calls itself canonical and `laws/INDEX.md` says so too, but neither matches the per-agent document that actually records proof — so "how much of this system is proven" has three answers and the roll-up is the least reliable of them. The gap runs the *safe* direction (the roll-ups under-report), which is exactly why it will keep being tolerated. **The larger number is not automatically right:** a 🟩 in a test-plan is only true if a passing test cites that ID, so closing this needs the S152-style per-row verification pass, not a copy of the bigger figure. | Next law-book pass, or the first time an agent's coverage ratio is quoted to justify a decision. Do it with row O in one sweep — same sweep, same script: assert every `laws.md` ID has a `test-plan.md` row (O), and that each 🟩 row cites a test that exists and names the ID (R), then regenerate both roll-ups from the test-plans rather than hand-maintaining them. |
| R² | **29 of the 284 🟩 rows in the book are not backed by what conventions §3 calls green.** Measured 2026-08-02 by parsing every `agents/*/laws/test-plan.md` and resolving each cited test against the live tree: **10 cite a test function that no longer exists anywhere** (`EXEC-IN-02`, `EXEC-TRG-03/05`, `MON-OUT-02/03/05`, `MON-NEV-01`, `PM-TYP-03`, `PROV-FAIL-02`, `PROV-OBS-03` — several survive only in the `mutants/` snapshot, deleted when ADR-0017 retired `execute_close` and the monitor's exit authoring), and **19 cite a live test whose docstring never names the clause ID**, which is §3's literal definition of green. Supervisor alone has 6. **A second, structural finding:** 7 of 14 agents use a `Clause \| Status \| Test` table with **no clause-summary column at all** (curator, forecaster, monitor, operator, reporter, researcher, supervisor), so [ADR-0021](decisions/0021-clause-summary-mirrors-the-law.md) has nothing to attach to in half the book, and their Test column carries bare function names with no file. | A green whose test was **deleted** is worse than the S151 rewording that prompted ADR-0021: there the proof was partial, here there is none, and nothing flagged it when the test went. The count is also a **ceiling, not a floor** — the check is mechanical (does a docstring name the ID?), so it cannot catch the S151 shape itself, a real citation on a test proving one third of its clause. | Same sweep as O and R. The order matters: **build the check first, then work the list to zero**, because the reverse is what produced this — S152 reconciled counters by hand and the drift reappeared one document down. Done when the check passes with each row either re-cited or honestly demoted to ⬜; **expect the book-wide green count to fall.** Schema unification is a separate later pass — the durable end-state is *generating* the summary column from `laws.md` so ADR-0021 is mechanically true rather than remembered. |
