...
Opened 2026-07-23 — **gates or agent authority**, found while shipping ADR-0016. **L closed 2026-07-27** (see Done); M and N remain. **O and P added 2026-08-01**, **Q added 2026-08-02** — both found while reconciling the law book in S152, both pre-existing and deliberately outside that sprint's scope. **R and R² closed in S156**; O remains open because assertion E is deliberately warn-only until S157 writes the missing rows. **T added 2026-08-04.** **S added 2026-08-03 and closed 2026-08-04** — its stated diagnosis turned out to be wrong and the correction is recorded in Done, because the wrong half ("only the aggregate differs") is what made it look unfixable.
...
| T | **Unit tests resolve *production* Service Bus credentials from `.env`, and a fresh dev environment cannot pass `make ci`.** `AzureServiceBusSettings` declares `env_file=".env"` (`kernel/bus_azure_config.py`), and pydantic-settings reads that file **directly, regardless of `os.environ`**. Measured from the repo root: `connection_string_for_topic("t")` returns a live `servicebus.windows.net` endpoint, so `AzureServiceBusBus.publish` takes the real `_azure_send` branch instead of the in-memory subscriber path. The two tests in `tests/test_deliberator_servicebus_peer.py` therefore depend on the **`azure` extra being installed** — and `make install` is a plain `uv sync`, which does not install it. Found 2026-08-04 while bumping cryptography; **it predates that change.** | Two distinct problems share one cause. **(1)** A clean checkout following the repo's own install step gets `ModuleNotFoundError: No module named 'azure'` and a red gate — my venv only passed because it carried extras from earlier work, which is the worst kind of green: it depends on the machine's history. **(2)** A unit test that resolves live credentials is one dependency-install away from transacting with production. **Stated honestly: I verified the live endpoint resolves and that the real-send code path is taken; I did NOT establish that a message reaches the namespace** — the test passes in 3.74 s without raising and that is unexplained in either direction. CI never sees any of it: there is no `.env` on the runner, so the same tests take the in-memory path and pass (2096 passed / 6 skipped on `cfee4c8`). A gate that is green only because the runner lacks a file is not proving what it appears to prove. | Next time `kernel/bus_azure*.py` or the deliberator peer tests are touched — or immediately if anyone reports a mystery Service Bus message. First move is to answer the open question (does a message actually land?), because it decides severity: a test-isolation cleanup if not, a production-safety fix if so. Likely shape either way: the tests should inject settings rather than let `env_file` reach ambient credentials, so the in-memory path is chosen explicitly instead of by accident of environment. |
