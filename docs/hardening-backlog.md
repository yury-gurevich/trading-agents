...
| **K** | Assertion-strength gap in trade-gating + money-parsing code (surfaced by S132 mutation) | **Partial in S134.** Round 1 kept the real money-parser and gate-boundary wins; Round 2 moved the S132 rerun from 5,440/6,731 killed (80.82%) to 5,567/6,731 killed (82.71%), with targeted analyst-domain survivors down from 249 to 127. The money-parser pure-helper bucket remains 39 -> 7 equivalent `_fill_from_order` default-normalization residuals, and the PM/gate/reward-risk/sector boundary tests remain real kills. | Returns to Done only after the remaining analyst scoring/indicator/alpha/technical survivors are killed or individually justified. Permanent justified non-targets remain limited to `# pragma: no cover` Alpaca HTTPS transport covered by live/paper broker proof and string/render/audit/context wording survivors excluded to avoid over-specification. Evidence: [S134 assertion hardening report](reports/sprint-134-assertion-hardening/README.md). |
