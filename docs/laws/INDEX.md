...
| --- | --- | --- | --- |
| provider | ✅ LOCKED v1.1 (S187) | 16 / 62 | S187 adds PARAM rows only; S156 citation check; counters are clauses proven / clauses declared; S204 gives every clause a row and makes rowless clauses a hard gate |
| analyst | ✅ LOCKED v1.2 (S186), amended **v1.3** (S198) | 25 / 48 | S198 declares and proves `ANLZ-OBS-05` — a recorded realized drawdown declares its horizon; S156 citation check plus S186's `ANLZ-OBS-04`; counters are clauses proven / clauses declared; S204 gives every clause a row and makes rowless clauses a hard gate |
| scanner | ✅ LOCKED v1.1 (S70, amended S183) | 18 / 41 | S156 citation check + S183's `SCAN-OUT-06`/`SCAN-OUT-07`; counters are clauses proven / clauses declared; S204 gives every clause a row and makes rowless clauses a hard gate |
| portfolio_manager | ✅ LOCKED v1 (S70), amended **v1.4** (S197) | 29 / 48 | S197 declares and proves `PM-OBS-03` — an evaluated concentration gate names what it examined; S184 made PM-NEV-07/08/09 green and closed the v1.3 widened PM-NEV-06 and PM-TYP-03 rows; S204 gives every clause a row and makes rowless clauses a hard gate |
| deliberator | ✅ LOCKED v1.5 (S200) | 19 / 55 | S200 adds `DLIB-OBS-07` (an `LLMCall` names the code that rendered its prompt) and turns `DLIB-IDM-02` green by hashing the **system** prompt too — **CORRECTING DRIFT-056** the way that row demanded, with every bound the clause names asserted, not just the repaired half (work-queue item 44); S199 adds `DLIB-OBS-05`/`DLIB-OBS-06`, so an `LLMCall` carries the provider's own token counts, says whether they are measured or estimated, and records the cached tokens a prompt-cache marker actually bought (work-queue item 43); S172 declares `debate_concurrency` and proves concurrent-order record order, bounded pending-reply waits, and pending sibling reply handling; S194 added `DLIB-OBS-04`, recording per-run orphaned peer reply counts on `DeliberationRun`; S189 added stop-reason audit evidence, stopped-completion failure, and the empty-turn prohibition; S167 added `DLIB-OBS-03` proof for queryable fail-open causes |
| execution | ✅ LOCKED **v1.4** (S190) | 35 / 61 | S190 adds/proves `EXEC-OBS-05`, adds the missing `EXEC-OBS-03` liveness-limb proof, and proves `EXEC-STA-05`; counters are clauses proven / clauses declared; S204 gives every clause a row and makes rowless clauses a hard gate |
| monitor | ✅ LOCKED v1 (S71) | 20 / 46 | S156 citation check; counters are clauses proven / clauses declared; S204 gives every clause a row and makes rowless clauses a hard gate |
| reporter | ✅ LOCKED v1 (S71) | 20 / 39 | S156 removed orphan `RPT-OBS-03`; S204 gives every clause a row and makes rowless clauses a hard gate |
...
| master | ✅ LOCKED v1.2 (S73, amended S188) | 15 / 44 | S188 adds and proves credential-test handover guards; RSA/Key Vault clauses deferred S74; S204 gives every clause a row and makes rowless clauses a hard gate |
