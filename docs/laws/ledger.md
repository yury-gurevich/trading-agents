...
| --- | --- | --- | --- |
| provider | ✅ v1.1 (LOCKED) | 16 / 62 | 🟨 partial — **16 of 62 clauses proven** after the S156 citation check; S187 added PARAM rows only, so counters did not move; S204 gives every clause a row and promotes missing rows to a hard gate; S169-sweep demoted `PROV-OUT-04` |
| scanner | ✅ v1.1 (LOCKED) | 18 / 41 | 🟨 partial — **18 of 41 clauses proven**: the S156 citation check plus `SCAN-OUT-06`/`SCAN-OUT-07` added and proven green by S183 (two clauses, three rows — two rows cite `SCAN-OUT-06`); S204 gives every clause a row and promotes missing rows to a hard gate |
| analyst | ✅ v1.2 (LOCKED, amended **v1.3**) | 25 / 48 | 🟨 partial — **25 of 48 clauses proven** after S198 adds and proves `ANLZ-OBS-05` (a recorded realized drawdown declares the horizon it covers — the outcome half ADR-0013 assumed and never built); S186 adds and proves `ANLZ-OBS-04`; S204 gives every clause a row and promotes missing rows to a hard gate · S152 declared 3 new clauses (43→46) · S169-sweep demoted `ANLZ-OBS-01` |
| forecaster | ✅ v1 (LOCKED) | 16 / 45 | 🟨 partial — **16 of 45 clauses proven** after the S156 citation check; 29 have a gray row |
| portfolio_manager | ✅ v1 (LOCKED, amended **v1.4**) | 29 / 48 | 🟨 partial — **29 of 48 clauses proven** after S197 declares and proves `PM-OBS-03` (an evaluated concentration gate reports its census, so a cluster of one is distinguishable from a comparison that never ran); S184 made PM-NEV-07/08/09 green and closed the v1.3 widened PM-NEV-06 and PM-TYP-03 rows; 12 law clauses still have no row (S157 warn-only backlog) |
| deliberator | ✅ v1.5 (LOCKED) | 19 / 55 | 🟨 partial — **19 of 55 clauses proven** after S200 adds `DLIB-OBS-07` (each `LLMCall` records a digest of the code that rendered its prompt, so replayability is answerable without replaying — 62 of 285 stored turns replay, re-measured 2026-09-13) and promotes `DLIB-IDM-02` ⬜ → 🟩 by hashing the system prompt alongside the user prompt, **CORRECTING DRIFT-056**; S199 adds `DLIB-OBS-05` (an `LLMCall` carries the provider's own token counts and declares them measured or estimated, truncated completions included) and `DLIB-OBS-06` (the frozen role prompt is offered to the prompt cache and the cached tokens are recorded), closing work-queue item 43 where the ledger wrote word counts excluding the system prompt; S172 proves deterministic concurrent-order record order, bounded pending-reply waits, and pending sibling reply handling; S194 adds `DLIB-OBS-04`, making per-run orphaned peer reply counts queryable from `DeliberationRun`; S189 added stop-reason audit evidence, stopped-completion failure, and the empty-turn prohibition; S167 proves queryable fail-open causes (`DLIB-OBS-03`) |
| execution | ✅ v1.4 (LOCKED) | 35 / 61 | 🟨 partial — **35 of 61 clauses proven** after S190 added and proved `EXEC-OBS-05`, added the missing `EXEC-OBS-03` liveness-limb proof, and proved the previously gray `EXEC-STA-05` row; S204 gives every clause a row and promotes missing rows to a hard gate · S185 declared and proved deliberation posture (`EXEC-OUT-09`, `EXEC-NEV-06`, `EXEC-OBS-04`); S187 added a PARAM row only; S152 declared 8 new clauses (49→57); `chore-exec-fail-03-coverage` proved EXEC-FAIL-03 · S169-sweep demoted `EXEC-OBS-01` and `EXEC-OBS-02` |
| monitor | ✅ v1 (LOCKED) | 20 / 46 | 🟨 partial — **20 of 46 clauses proven** after the S156 citation check; S204 gives every clause a row and promotes missing rows to a hard gate |
| reporter | ✅ v1 (LOCKED) | 20 / 39 | 🟨 partial — **20 of 39 clauses proven** after the S156 citation check; S204 gives every clause a row and promotes missing rows to a hard gate |
...
| master | ✅ v1.2 (LOCKED) | 15 / 44 | 🟨 partial — **15 of 44 clauses proven** after S188 adds and proves credential-test handover guards; S204 gives every clause a row and promotes missing rows to a hard gate · RSA signing + Key Vault + integration clauses deferred (S73/S74) |
