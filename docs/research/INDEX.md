...
| [R006](code-quality-tooling/) | Code-quality & test-effectiveness tooling — PyExamine + the unused test-effectiveness methods | 2026-07-22 | 🗄️ Archived | Should we adopt PyExamine? Which of coverage / mutation / defect-detection-rate / pseudo-tested / flakiness do we not use, and do we need them? | **Nothing adopted.** PyExamine overlaps an already-*enforcing* gate (import-linter + 200-line cap + ruff/mypy) and would flag our deliberate cross-agent duplication as a smell. Drift-register classification shows **no recorded production defect was a decision-engine logic defect** — so the one carried-forward action is a retrospective **defect-detection-rate** analysis (no tooling); pseudo-tested parked, flakiness skipped | `code-smells` `test-effectiveness` `mutation-testing` `pyexamine` `tooling` `signal-to-noise` |
| [R007](ontology-guardrails/) | Ontology guardrails (RDFS / OWL) for the agent loop | 2026-07-27 | 🗄️ Archived | Should the agent loop be tethered to a formal ontology (W3C RDFS/OWL), per Coyle's *Why Agentic Systems Need Ontologies*? What would it add on top of the Pydantic layer we already have? | **Take "OWL constrains", refuse "RDFS infers"** — reasoner ruled out permanently (inference asserts unobserved facts; incompatible with LAW-02 and the append-only evidence store). Constraint half recommended in ~100 lines of first-party Python. Finding: `labels_owned` is declared in **8** law files and read by **no code** | `ontology` `rdfs` `owl` `knowledge-graph` `neuro-symbolic` `graph` `laws` `law-02` `adr-0014` |
| [R008](survivorship-free-universe/) | Survivorship-free S&P 500 universe — point-in-time membership + delisted bars (P17 E17.1) | 2026-09-25 | ✅ Measured | Can we source point-in-time S&P 500 membership and removed-name bars on our plans? Is ~90 % member-day coverage met? | [DL-226](../design-log.md): Wikipedia change log + Alpaca SIP, 95.3 % (≈99 % with renames); FMP/Finnhub ruled out | data, backtest, survivorship |
...
| [code-quality-tooling/](code-quality-tooling/INDEX.md) | R006 | PyExamine + test-effectiveness methods — nothing adopted; defect-detection-rate analysis carried forward. |
| [textgrad/](textgrad/INDEX.md) | R003 | TextGrad evaluated for continuous improvement — not adopted; ADR-0010 bake-off candidate. |
| [survivorship-free-universe/](survivorship-free-universe/INDEX.md) | R008 | E17.1 measurement: membership source, bar coverage, identity check, and what E17.2 must build. |
