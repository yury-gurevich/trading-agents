...
| --- | --- | --- | --- | --- | --- | --- |
| [R001](qlib-integration/) | Microsoft Qlib — integration vision + workflow addendum | 2026-06-19 | 🚧 In progress | Can this project benefit from qlib? Which agents, which components, which *workflows*, in what order? | Q1 (S58–S59) + Q2 (S68) + Q1b (S110) + Q1c (S111) + Q3 (S112) complete. **Q5 part A complete (S113, 0.57.00 — governed factor proposal, LLM enum-guarded)**; remaining: Q5 part B (S115, shadow→promote) closes the loop; Q4 behind live-data prerequisite | `qlib` `forecaster` `analyst` `portfolio_manager` `researcher` `ml` `backtesting` |
