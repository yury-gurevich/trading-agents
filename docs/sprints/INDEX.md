...
| **Operations dashboard (DL-47)** | S122 (queued) · S123–S125 (to package) | The PRD's product surface #1: `surfaces/dashboard/` read-model service + static frontend. S122 = run selector + Section III trading-run logical verdicts (observatory-sourced) + flags/positions/recovery ladder; S123 = fleet lifecycle + infra + per-container logs (Log Analytics); S124 = resume-from-stage (Airflow clear+downstream semantics over graph-pull artifacts) + DL-46 tripwire; S125 = operator-agent chat. Design spec: `docs/design/dashboard-mockup.html`. | **queued** — S122 packaged 2026-07-10 |
...
1. Next number: **S123** (S122 dashboard run view is packaged/queued; S123–S125 are reserved for
   the remaining DL-47 dashboard slices — fleet lifecycle + per-container logs, resume-from-stage,
   operator chat).
