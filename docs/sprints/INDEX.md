...
| **Q1c** | S111 | Qlib workflow adoption: rolling retrain + IC-decay trigger — decay measured against the S110 baseline, challenger must beat incumbent on the same window, operator holds `--apply`; + committed resumable Tiingo exporter | **shipped on branch** — 0.54.00, pending operator review/merge |
| **Q3** | S112 | Qlib workflow adoption: researcher backtest evidence — self-built no-lookahead walk-forward harness (pyqlib 3.13-incompatible), `BacktestEvidence` optional contract field, bounded signal-catalogue CLI; the Q5 prerequisite | **packaged** — execute after S111 is reviewed and merged; then Q5 governed factor mining |
...
| **ADR-0010 (LLM quality)** | S109 | Heterogeneous deliberation: Defender/Challenger on GPT-5.5, the debate Judge on Opus (`claude-opus-4-8`); dedicated `DELIBERATION_JUDGE_*` env (operator model untouched); veto now debates a grounded proposition (fixes S96). Not the EXP-004 scorer judge. | **shipped 0.52.00** (2026-07-03) — **live-Opus check + golden re-freeze DEFERRED** (Anthropic billing); functional via a temporary gpt-5 judge; re-run scheduled |
