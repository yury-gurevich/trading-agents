...
| **Operations dashboard (DL-47)** | S122 shipped · S123 complete on branch · S124–S125 next | The PRD's product surface #1: `surfaces/dashboard/` read-model service + static frontend. S122 = run selector + Section III trading-run logical verdicts (observatory-sourced) + flags/positions/recovery ladder; S123 = fleet lifecycle + infra + per-container logs (Log Analytics) + A$ cost meters; S124 = resume-from-stage (Airflow clear+downstream semantics over graph-pull artifacts) + DL-46 tripwire; S125 = operator-agent chat. Design spec: `docs/design/dashboard-mockup.html`. | **active** — S123 complete on `sprint-123-dashboard-fleet-infra`, pending merge |
...
1. Next number: **S124** (S122 shipped and S123 is complete on branch; S124–S125 are reserved for
   the remaining DL-47 dashboard slices — resume-from-stage + tripwire,
   operator chat).
