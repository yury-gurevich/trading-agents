...
| [sprint-121](sprint-121-judge-promotion-challenger-recompile.md) | S119 resolution: promote the compiled judge into `JUDGE_SYSTEM` (first ADR-0010 prompt promotion) + mandatory golden re-freeze; one bounded challenger recompile gated against the new baseline (keep-champion is a valid outcome); defender untouched | **planned** (handover Codex-ready; after S120) |
| [sprint-120](sprint-120-broker-reconciliation.md) | Broker reconciliation (DL-44): broker = truth for holdings, graph = truth for lineage — execution-owned run-start `BrokerPositionSnapshot` + loud divergence Flag, monitor adopts broker truth, pending-Fill refresh, teardown discipline amended; live check repairs the CSCO double-buy divergence | **branch complete** (0.65.00; not merged/pushed) |
