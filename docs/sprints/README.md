...
| [sprint-106](sprint-106-remediation-planner.md) | DL-36 Piece C: LLM remediation planner (bounded catalogue) — on an `Escalation`, the LLM selects a vetted remediation + justifies; records a `RemediationPlan` + `auto_eligible` (configurable `auto_remediation_scope`); plans+gates, never executes (D next) | **planned** |
