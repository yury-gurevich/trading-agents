...
| [sprint-104](sprint-104-credential-tested-activation.md) | Credential-tested activation (DL-36 A+B): master tests every credential before handover (cheap live + cache costly); a required failure refuses activation (fail-safe) + writes an `Escalation` with the one-shot counter | **planned** |
