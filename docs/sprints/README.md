...
| [sprint-111](sprint-111-rolling-retrain.md) | Rolling retrain + IC-decay trigger (qlib Q1c): committed resumable Tiingo exporter, pure `retrain_policy` (decay verdict + champion-vs-challenger), retrain pipeline CLI — dry-run default, `--apply` swaps with archive-never-delete | **shipped on branch** (0.54.00; pending operator review/merge) |
| [sprint-112](sprint-112-researcher-backtest-evidence.md) | Researcher backtest evidence (qlib Q3, self-built): pure no-lookahead walk-forward harness in researcher domain, `BacktestEvidence` optional contract field (0.2.0), bounded signal-catalogue evidence CLI — prospective Sharpe/IC alongside provenance on proposals | **packaged** (after S111 review/merge) |
