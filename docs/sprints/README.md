...
| [sprint-78](sprint-78-provider-ingestor.md) | Provider as standalone graph-ingestor: replace `idle_loop()` with real ingest loop; `build_graph_from_env()` kernel helper; provider polls DB and writes DataNode/NewsItem; "one container start → data in graph" proof; version 0.17.0 (MINOR) | **planned** |
| [sprint-79](sprint-79-agent-work-loops.md) | Agent work loops — "check graph → do work": `find_pending()` per agent (Cypher query for unprocessed work) + `work_loop()` kernel helper; all 6 trading-pipeline agents replace idle_loop; "one container at a time" full-pipeline proof; version 0.18.0 (MINOR) | **planned** |
