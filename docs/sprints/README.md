...
| [sprint-128](sprint-128-feed-resilience.md) | Feed resilience (DRIFT-021): per-request Finnhub pacing (55/min tunable budget), per-ticker fault attribution in all four enrichment feeds (DRIFT-014 pattern — one 429 costs one ticker, not the feed), durable attributed notes on the graph quality trace; live proof uses the real rate limit as fault injector | **code proof complete on branch** (0.71.01); live proof blocked by DRIFT-023 / Neon `OperationalError` |
