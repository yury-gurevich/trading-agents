...
| [sprint-76](sprint-76-p15-ghcr-deploy.md) | P15 GHCR build pipeline + Container Apps deploy: GitHub Actions matrix build → GHCR images; Key Vault provisioning + secret seeding; deploy-agents workflow (master first, then parallel); `trading-agents-kv` live; scanner boots + EHLO confirmed in log stream | **planned** |
