...
| [sprint-123](sprint-123-dashboard-fleet-infra.md) | Operations dashboard slice 2 (DL-47): Sections I+II — Azure read-port (Container Apps state, job executions, Log Analytics per-container logs, Cost Management), fleet lifecycle projection, hardware+LLM cost meters (`LLMCall` ledger x `llm_pricing.json` pack), `/api/vitals`, bundle gains real `logs`+`images` | **queued** |
