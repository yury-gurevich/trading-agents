...
| [sprint-111](sprint-111-rolling-retrain.md) | Rolling retrain + IC-decay trigger (qlib Q1c): committed resumable Tiingo exporter, pure `retrain_policy` (decay verdict + champion-vs-challenger), retrain pipeline CLI — dry-run default, `--apply` swaps with archive-never-delete | **packaged** |
