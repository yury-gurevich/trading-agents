...
| 29 | **Six behavioural settings are bare defaults, not `tunable()`s** — *[swept 2026-08-20, [DL-120](design-log.md)]* | Sweep of all **24 settings classes / 246 fields**: 236 properly declared, 32 wiring, **10 behavioural suspects**. 🚨 **The pattern matters more than the count** — two agents made the *same* mistake: `analyst.stop_target_mode` (S150) and `execution.order_price_tolerance_mode` (S149) are both bare mode switches **surrounded by properly-registered tunables that configure the challenger nobody can select**. Each sprint registered the parameters and forgot the switch. Also bare: `scanner.benchmark_ticker` (drives every `relative_strength` and `beta`), `provider.alpaca_data_feed` (IEX is a *partial* feed), `provider.ingest_ohlcv_only`, `curator.predictor_strategy`. `execution.stage` needs a `why` but is **not dangerous** — the graph is authoritative and `run.py:51` rejects every non-paper stage. 🟢 One-line registrations, no behaviour change; keep every default as-is. 🪤 `stop_target_mode` is already in S183 — **do not double-fix it**. 🪤 Bare literals *inside domain code* (the S174 `_DEFAULT_LOOKBACK_DAYS = 60` class) were **not** swept | small |
