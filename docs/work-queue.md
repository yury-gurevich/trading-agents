...
| 17 | 🚨 **45 unresolved `critical` Flags pin `healthy=false`** — *packaged as S178, 2026-08-15* | Since 2026-07-08. 🪤 **Re-measured:** 45, not 47, and **2 criticals have been resolved** — the machinery works. The backlog only grows because `subject_ref` embeds a per-run timestamp (dedupe never fires) and only the human `approve` capability resolves. 🟢 Reporting-only; acceptance reads neither field | 1 small sprint + one sweep |
