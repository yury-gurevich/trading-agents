...
| 18 | **`max_sector_pct` never sees held positions** — the *behaviour* half, still open | 🪤 **The label half is CLOSED by S177** (`deployed_this_batch_usd` now says what it counts). What remains is a real question: `SectorBook.__init__` seeds `_names` from held positions but not `_deployed`, so the dollar sector cap only ever counts the current batch and cross-day exposure can pass 30 % unnoticed. Deliberately split out in [DL-113](docs/design-log.md) because seeding it **changes which orders get approved**. Masked today by `max_names_per_sector=3` binding first. 🚨 **WIDENED 2026-08-19 — the deliberator has now named this on three consecutive nights, and the gap is bigger than `_deployed`:** there is **no issuer or correlation dimension at all**. GOOG vetoed because *"the PM treats GOOG as a new ticker while the portfolio already holds GOOGL"* (dual-class, same issuer); INTC because it *"would add to existing AMD/NVDA semiconductor exposure"* with no name-correlation penalty; AMZN because the Retail cap tests declared sector only, so AWS/AI exposure passes untested. Same objections on 08-18 (AVGO, GOOGL) and 08-14. **4 of the 6 vetoes on the clean run were this.** Scope the ADR to the whole question, not just seeding `_deployed` | ADR + small |
| 19 | ~~**`open_incidents` is the only remaining lock on `healthy`**~~ **DONE** (`S179`, `0.90.14`, 2026-08-18) — merged at `4c8eeb0`, gate proven (Security Findings · CodeQL · CI · Build/push all green). **`healthy` is `true` on the live spine for the first time since 2026-07-08.** Incidents are now scoped rather than all-time; retirement appends a `FaultResolution` (2 written, `resolved_by=s179-fault-incident-sweep`). **Verified independently:** `Fault` **6119 → 6119**, all still `status=pending` — nothing mutated or deleted; the `occurred_at` keying is untouched; and **both** predicate copies agree on the same graph (`health.py` and `surfaces/queries/health.py`), which was the flagged silent-half-ship risk. 🪤 **Fleet not yet on `s179`** — see item 20 | ~~1 small sprint~~ |
| 20 | **Stop-identity mismatch re-faults every run** — *[measured 2026-08-18]* | **112 Faults**, all `severity=warning`, `BrokerStopIdentityMismatch` from [`drop_sweep_records.py:64`](agents/execution/drop_sweep_records.py), across **12 tickers** (ABT AMZN AVGO BAC BMY CSCO HPE MDT PYPL SCHW USB WFC) on **8 distinct days** since **2026-08-08**, at ~10–12 per run. **Measured condition:** the broker holds the stop open (`broker_stop=True`) while the graph `Fill` for the same idempotency key reads `broker_status=rejected` (`graph_stop=False`) — e.g. `stop:41995c05a31800d7:BMY` is `rejected` in the graph at qty **153**, a quantity from a pre-flatten lot, while a live BMY stop sits at the broker. The sweep **exempts** the order rather than cancelling it, which is the safe behaviour. **Not yet determined:** whether the idempotency key is being reused across lots or the graph missed a status refresh — decide that before changing anything. 🟢 Warning-level only and deliberately **not** hidden by S179; it no longer pins `healthy` | 1 small sprint |
| 21 | **`record_deploy.py` trusts the SHA it is handed** — *packaged as [S180](docs/sprints/sprint-180-a-deploy-record-must-name-the-commit-that-was-built.md), 2026-08-18* | Recording the `s179` deploy with `--git-sha $(git rev-parse HEAD)` wrote the **wrong** commit: `HEAD` had moved one docs-only commit past `4c8eeb0`, the commit the images were actually built from. `projections_currency.py:61` compares the record’s `git_sha` against the **latest main image build**’s SHA, so the dashboard would have read the fleet as **"behind" while it was current** — the exact DL-46 currency error the record exists to prevent. Caught only because the printed SHA was read, same as the `make gate-ran` worktree trap. **Fix:** have `record_deploy.py` resolve or **verify** the SHA against the newest `build-images.yml` run and refuse on mismatch, rather than trusting the argument. 🟢 Corrected by appending a second record; the bad one is superseded, not erased | small |
| 22 | **A sprint’s built/unbuilt state is not machine-checkable** — *[measured 2026-08-18]* | Closeout placeholders use **at least three different wordings** across specs (`FILL THIS IN BEFORE HANDING BACK`, `Coding agent: replace this comment…`, and S171’s closeout which opens `**Correlation.**` with no `Result:` line at all). There is no single marker that answers "is this spec built?", so **I recommended handing Codex S171 on 2026-08-18 when it had already shipped as `9a22102` / `v0.90.01`** — caught only because Codex checked git ancestry. **Fix:** one required machine-checkable line in the closeout (e.g. `**Status:** SPEC | BUILT`), plus a CI or `/where` check that reconciles it against the INDEX status column. 🟢 Docs-only; the cost is wasted handovers and wrong ranking, not broken code | small |
| 23 | ~~**One canceled test order red-lights the whole fleet, every night**~~ **DONE** (`S181`, `0.90.15`, 2026-08-19) — [DL-115](design-log.md). The drop sweep now writes a durable, edge-less `BrokerOrderStatus` ack (`lineage_status="missing_fill_ack"`) on first sight and consults it only while the `Fill` is still absent, so Fill precedence is structural. First-sight `error` kept; ack-only orders still return `False` so `ExecutionRun.dropped` is unchanged. No vocabulary-pack move — `BrokerOrderStatus` is already declared and is not one of the 5 property-enforced labels. `make ci` **2322 passed / 100.00 %**, 7 guards planted and restored, `GATE PROVEN` for `35b0c9d`. 🪤 **Closeout module-size figures were non-blank counts**; the gate counts total lines, and three files remain over the 150 warn line — corrected at merge. **Pending:** deploy, then one `FaultResolution` for the existing incident | `healthy` went `true` on 2026-08-18 (S179) and was `false` again at the next run. **Measured 2026-08-19:** `live_fault_incidents` returns exactly **one** node — `UntrackedOpenOrder`, `error`, from `drop_sweep`, for `stop:probe-s164:T#1`: a **1-share S164 probe** at Alpaca, submitted 2026-08-07T08:50:21Z and canceled 0.7 s later, with no `Fill` node. **12 identical Faults since 2026-08-08**, one per run. The loop is closed: `_already_dropped` reads `fill.props["drop_reason"]` and there is no Fill, so **the only memory the sweep has lives on a node that does not exist** — nothing on the path writes anything and the next run repeats it exactly. Of 197 broker orders, 192 are pipeline-owned, 103 terminal, exactly **1** without a Fill chain. 🪤 **Cannot be torn down** — the order is already `canceled`, terminal and immutable at Alpaca; 🪤 a `FaultResolution` sweep only retires the fault already written; 🪤 it self-heals once 500 newer orders push it out of the `limit=500` window, flipping health green on **order volume**. 🟢 Reporting-only; acceptance reads neither field | small |
| 24 | 🚨 **Nine orders reached the broker unvetoed because the debate finished 71 s late** — *[measured 2026-08-19]* | On `sched-2026-08-19` PM approved **9**; execution's grace expired and it submitted **all 9** at 07:02:17–22 with `deliberation_status=proceeded_unvetoed`, and the `DeliberationRun` landed **07:03:33** — then returned **6 vetoes**. `ACCEPTANCE FAIL`: `debate_coverage` **0.778 < 1.0**, `failed_open_count` **2 > 0** (`no deliberator peer reply received`, MO + CSCO). 🪤 **The 7-order run the night before cleared the same grace comfortably** — the margin is thin enough that two extra orders plus two peer failures break it, so this is not a far-off scaling limit, it is the current operating point. This is the concrete cost item 3 was missing: **not throughput, but orders reaching the broker unreviewed.** 🟢 S175 worked — the unvetoed submission raised its own queryable error fault. Fold into **S172**, or ship the `max_rounds`/`effort` sweep first as the cheap mitigation. ✅ **MITIGATED 2026-08-19 (DL-116):** grace 900 → 1800; the next run came back `applied_failed_open`, not `proceeded_unvetoed` — **the veto bound and blocked 6 of 9**. 🪤 At ~15 orders the raised per-call timeout and the grace collide again (~1650 s vs a `le=3600` cap), so S172 is still the fix — now measured at **15 orders**, see item 3 | see item 3 |
| 25 | 🚨🚨 **BLOCKER — the deliberator has no working LLM provider** — *[measured 2026-08-19]* | Probed directly: **OpenAI `HTTP 429` "You have no credits remaining"**; **Anthropic `HTTP 400` "You have reached your specified API usage limits. You will regain access on 2026-09-01"**. Both are down, so **every debate fails open** and no run can pass acceptance (`failed_open_count > 0` fails on its own). Measured across two runs today: 2 then 3 fail-opens, coverage 0.778 then 0.667. 🪤 **It presents as a latency problem and is not one** — the fail-open faults first read `no deliberator peer reply received`, and the LLM latency tail happened to sit at the configured 60 s ceiling, so it looked like a timeout (DL-116, amended). The real reason string was in the `DeliberationRun.failed_open_reason` field the whole time. **Fix is not ours:** add OpenAI credits, or wait for the Anthropic reset on **2026-09-01**. 🟠 **PARTIALLY LIFTED 2026-08-19** — operator added $5; OpenAI reads `HTTP 200` and `verify-2026-08-19-clean-2` came back clean (9/9 debates, 0 fail-opens, `applied`) for **$0.46**, so ~10 runs of headroom. **Stays open:** Anthropic is still capped until 2026-09-01, so there is **no working fallback provider** and the next credit exhaustion stops the fleet exactly the same way, with the same misleading `no deliberator peer reply received` symptom. 🟢 Nothing else is broken — the pipeline runs 8/8 and the veto now binds | operator action |
| 26 | **News is attributed to tickers it is not about, and it moves the sentiment score** — *[measured 2026-08-19, cited on 3 nights]* | The deliberator keeps rejecting sentiment as evidence because the headlines behind it are not about the name. Clean run, GOOGL: *"the sentiment input is visibly polluted with unrelated headlines, so the narrow confidence pass may be an artifact of contaminated input rather than a clean buy signal."* Same complaint hit **AMZN, XOM and CSCO on `sched-2026-08-18`** (*"mostly broad S&P/AI/Nvidia headlines with only one AMZN-specific headline"*). 🚨 **This is not cosmetic** — `analyst_sentiment_score` feeds the confidence gate, and several approvals cleared the floor **on sentiment while technicals were weak**, so mis-attributed news is buying stocks. 🪤 **Distinct from DL-112**, which was a unit-naming bug: the counts are right, the *articles* are the wrong articles. **Not yet diagnosed** — unknown whether the provider returns loosely-matched results or the analyst does not filter them; measure that before designing anything | diagnose first |
---
