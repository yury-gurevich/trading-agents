...
| 19 | 🚨 **`open_incidents=6104` is now the only thing pinning `healthy=false`** — *[measured 2026-08-18]* | S178 cleared the flag half (46 → 0) and `healthy` **still** did not move: `compute_health` is `open_incidents == 0 and critical_flags == 0`. **All 6104 Faults are `status=pending` — there is no retirement path at all**, the same disease [DL-111](docs/design-log.md) just fixed for flags. 🪤 **But the number is misleading: 5762 of 6104 (94 %) are one closed incident** — `property 'broker_status' cannot be overwritten` (`ValueError`, `agents.execution.poll`, capability `position_sync`), confined to **2026-07-30 (4125) and 2026-07-31 (1637)** and never seen since. **The live leak is only ~12/day**: `stop identity mismatch` on ~10 stale stop nodes (BMY/MDT/ABT/USB/WFC/PYPL/HPE/SCHW/BAC), re-faulted once per run because `_fault_key` embeds `run_id` — dedupe never fires across runs, exactly the flag bug. So this is **two items**: a retirement path, and the recurring stop-identity re-fault | 1 small sprint |
---
