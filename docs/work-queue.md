...
| 26 | **News is attributed to tickers it is not about, and it moves the sentiment score** | *[measured 2026-08-19, cited on 3 nights]* GOOGL vetoed: *"the sentiment input is visibly polluted with unrelated headlines, so the narrow confidence pass may be an artifact of contaminated input."* Same complaint hit **AMZN, XOM and CSCO on `sched-2026-08-18`**. 🚨 **Not cosmetic** — `analyst_sentiment_score` feeds the confidence gate, and several approvals cleared the floor **on sentiment while technicals were weak**. 🪤 **Distinct from DL-112**, which was a unit-naming bug: the counts were right, here the *articles* are wrong. **Not yet diagnosed** — unknown whether the provider loosely matches or the analyst fails to filter | diagnose first |
