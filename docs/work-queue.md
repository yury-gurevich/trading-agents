...
| 24 | 🚨 **Nine orders reached the broker unvetoed because the debate finished 71 s late** — *[measured 2026-08-19]* | On `sched-2026-08-19` PM approved **9**; execution's grace expired and it submitted **all 9** at 07:02:17–22 with `deliberation_status=proceeded_unvetoed`, and the `DeliberationRun` landed **07:03:33** — then returned **6 vetoes**. `ACCEPTANCE FAIL`: `debate_coverage` **0.778 < 1.0**, `failed_open_count` **2 > 0** (`no deliberator peer reply received`, MO + CSCO). 🪤 **The 7-order run the night before cleared the same grace comfortably** — the margin is thin enough that two extra orders plus two peer timeouts break it, so this is not a far-off scaling limit, it is the current operating point. This is the concrete cost item 3 was missing: **not throughput, but orders reaching the broker unreviewed.** 🟢 S175 worked — the unvetoed submission raised its own queryable error fault. Fold into **S172**, or ship the `max_rounds`/`effort` sweep first as the cheap mitigation | see item 3 |
---
