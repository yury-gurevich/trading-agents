...
| 19 | 🚨 **`open_incidents=6104` is now the only thing pinning `healthy=false`** — *[measured today]* | S178 cleared the flag half (46 → 0) and `healthy` **still** did not go green, because `compute_health` is `open_incidents == 0 and critical_flags == 0` and there are **6104** unresolved `Fault` nodes. Same disease as the flags: faults accumulate with no retirement path, so the boolean cannot move. Until this is fixed the health signal carries no information and S178's value is unrealised | 1 small sprint |
---
