...
**Re-reviewed 2026-08-31 13:40 AEST** against `main` @ `6b4463c` (`0.94.01`) and the fleet on `s187`.
This review **closed item 34** (shipped the hour it was ranked), **folded item 20 and item 12's Fill
half into item 32**, re-measured all three against the live spine, and promoted the merged row above
item 33. Items 3, 6b and 36 are unchanged: all three are gated on `sched-2026-08-31`.
...
| 32 | 🚨 **One liveness question, asked four ways — packaged as [S190](sprints/sprint-190-one-liveness-question-one-answer.md), and it now carries items 20 and 12's Fill half** | *[re-measured 2026-08-31, [DL-139](design-log.md)]* Execution's graph facts each denormalise lifecycle differently and only `Position` has a predicate hiding it (`contracts/positions.py:126`). Three faces: a `BrokerStopOrder` is active iff `cancelled_at is None`, so a **fired** stop stays active forever ([DL-131](design-log.md)); the stale-order sweep compares order **type** against graph **lifecycle**, so every dead stop mismatches forever ([DL-130](design-log.md)); `Fill.status` is an immutable submit-time fact with no predicate ([DL-129](design-log.md)). 🎯 **The re-measurement decided the design: the graph already holds the answer.** **All 46** `BrokerStopOrder` nodes have a `Fill` at the *identical* key — today `stop:87403939105c0a24:PYPL` reads live while its own Fill has said `broker_status=filled` since 08-28 (`realized_pnl_cents=-9758`). So the fix is a **predicate, not a new property**: no graph write, no vocabulary change, retag-safe. 🚨 **Two carried numbers were wrong.** DL-130's *"13 objects, not a growing fault"* is **17** seven days later (304 faults, up from 228) — the object count grows by one per stop that dies. And DL-129's leftover *"24 open Fills carry no `submitted_at`"* is not a mystery: **28 of the 29 open Fills are resting stops**, pending by design forever, and exactly **1** is a real order. 🪤 **7 stops carry `cancelled_at` written 1-3 days *after* their fill was recorded**, so a stop-out is logged as *cancelled by us* — the lifecycle has one word for three endings. 🪤 `partial` must stay non-terminal or S176 is re-broken. [DRIFT-055](laws/drift-register.md) filed: `EXEC-OBS-03` is 🟩 green on five tests, none of which exercise its *"the broker remains truth for liveness"* limb. 🟢 No capital at risk — `position_ref` is content-derived, so a stale active stop cannot suppress a new one | **SPEC ready to hand over**, 1 sprint, replaces 3 patches |
...
| 12 | **`pg_teardown`'s delete path has never run on live data** — *item 12's Fill half moved into item 32* | *[carried]* The Fill-backlog half is **superseded**: re-measured 2026-08-31 it is a vocabulary defect, not a backlog, and it is now S190's job ([DL-139](design-log.md)). ✅ The flag half closed earlier — 53 Flags have 53 `FlagResolution`s, `pending_human_flags` is 0. What is left is one thing: the delete path has never been exercised against real data, and its first real use must be a **disposable synthetic run**, not a live one | small, but do it deliberately |
