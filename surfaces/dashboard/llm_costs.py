...
class _AgentUsage(TypedDict):
    """Mutable per-agent pricing accumulator."""
...
def llm_cost_projection(
    ...
    *,
    ...
    for node in graph.list_nodes("LLMCall"):
        ...
        calling_agent = str(node.props.get("calling_agent", "unknown"))
        tokens_in = int(str(node.props.get("tokens_in", 0)))
        tokens_out = int(str(node.props.get("tokens_out", 0)))
    ...
    return {
        ...
        "fx": fx,
        "models": models,
        "agents": _agent_rows(agents, aud_per_source),
...
def _record_agent_usage(
    ...
    *,
    ...
    if price is None:
        ...
        return
    row["source_cost"] += (
        Decimal(tokens_in) * Decimal(str(price["input"]))
        + Decimal(tokens_out) * Decimal(str(price["output"]))
    ) / _MTOK


def _agent_rows(
    agents: dict[str, _AgentUsage], aud_per_source: Decimal
) -> list[dict[str, object]]:
    rows: list[dict[str, object]] = []
    for calling_agent in sorted(agents):
        usage = agents[calling_agent]
        source_cost = usage["source_cost"]
        rows.append(
            {
                "calling_agent": calling_agent,
                "calls": usage["calls"],
                ...
                "cost": round(float(source_cost * aud_per_source), 6),
                "source_cost": round(float(source_cost), 6),
            }
        )
    return rows
